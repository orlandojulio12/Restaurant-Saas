...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
API del SaaS de Restaurante Dinámico.

**Autenticación.** Todo salvo el menú público, el pedido por QR y el webhook de
WhatsApp requiere `Authorization: Bearer <token>`. El token se obtiene en
`POST /auth/login`.

**Aislamiento entre restaurantes.** El `restaurant_id` se deduce del token; nunca
se envía en la petición. Acceder a un recurso de otro restaurante devuelve 403.

**Fechas.** Los parámetros y respuestas con fecha usan el día local del
restaurante en formato `YYYY-MM-DD`, no UTC.

**Funciones de plan.** Los módulos de inventario, reportes y finanzas devuelven
403 si el plan del restaurante no los incluye.
MD,
    ],
