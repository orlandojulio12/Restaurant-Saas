# CLAUDE.md — Backend (restaurant-api)

API en Laravel 12 para un SaaS multi-restaurante: gestión de menú, mesas con QR,
pedidos en tiempo real (cocina/mozos), pagos, inventario por ingredientes, reportes,
módulo financiero y bot de WhatsApp.

El frontend está en `../restaurant-front`, con su propio CLAUDE.md. **Cuando cambie
un contrato de la API —campos de un Resource, un enum, un código de error— hay que
mirar si el frontend lo consume**: sus tipos son un espejo escrito a mano de estos
Resources, y la mayoría de fallos entre las dos capas han salido de ahí.
